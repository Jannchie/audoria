export const messages = {
  'en-US': {
    auth: {
      loginPrompt: 'Enter your access token to continue',
      tokenPlaceholder: 'Access token',
      tokenLabel: 'Access token',
      goBack: 'Go back',
    },
    nav: {
      library: 'Library',
      playlists: 'Playlists',
      explore: 'Explore',
      parse: 'Parse',
      upload: 'Upload',
      player: 'Player',
      settings: 'Settings',
      login: 'Log in',
    },
    common: {
      unknown: 'Unknown',
      or: 'or',
      actions: {
        search: 'Search',
        searching: 'Searching',
        parseUrl: 'Parse URL',
        parsing: 'Parsing',
        reset: 'Reset',
        download: 'Download',
        downloading: 'Downloading...',
        upload: 'Upload',
        uploading: 'Uploading...',
        login: 'Login',
        save: 'Save',
        saving: 'Saving...',
        cancel: 'Cancel',
        clear: 'Clear',
        close: 'Close',
        editMetadata: 'Edit metadata',
        deleteTrack: 'Delete track',
        deletingTrack: 'Deleting track',
        playNext: 'Play next',
        addToQueue: 'Add to queue',
        removeFromQueue: 'Remove from queue',
        addToPlaylist: 'Add to playlist',
        newPlaylist: 'New playlist...',
        moreOptions: 'More options',
        removeFromPlaylist: 'Remove from playlist',
        chooseImage: 'Choose image',
        remove: 'Remove',
        keepCurrent: 'Keep current',
        play: 'Play',
        pause: 'Pause',
        previousTrack: 'Previous track',
        nextTrack: 'Next track',
        mute: 'Mute',
        unmute: 'Unmute',
        seek: 'Seek',
        volume: 'Volume',
        copy: 'Copy',
        copied: 'Copied',
        failed: 'Failed',
      },
    },
    errors: {
      parseFailedStatus: 'Parse failed ({status})',
      updateFailedStatus: 'Update failed ({status})',
      coverUploadFailedStatus: 'Cover upload failed ({status})',
      coverDeleteFailedStatus: 'Cover delete failed ({status})',
      missingImportJobId: 'Missing import job id',
    },
    settings: {
      title: 'Settings',
      description: 'Manage interface preferences for this browser.',
      groups: {
        runtime: 'Runtime configuration',
        appearance: 'Appearance',
        playback: 'Playback effects',
      },
      sections: {
        label: 'Settings sections',
      },
      on: 'On',
      off: 'Off',
      language: {
        title: 'Interface language',
        description: 'Changes apply immediately and are stored locally on this device.',
        followSystem: 'Follow system',
        currentValue: 'Currently using {language}',
        effectiveLocale: 'Effective language',
      },
      coverEffectEnabled: {
        title: 'Cover foil effect',
        description: 'Turn off to hide the holographic overlay on the player cover.',
      },
      coverEffect: {
        title: 'Player cover style',
        description: 'Choose the foil style used on the player cover.',
        currentValue: 'Current effect: {effect}',
        options: {
          amazingRare: 'Amazing Rare',
          radiantHolo: 'Radiant holo',
          vMax: 'VMAX',
          vStar: 'VSTAR',
        },
      },
      progressEffectEnabled: {
        title: 'Progress bar shader',
        description: 'Turn off to use a minimal progress bar instead of the animated shader.',
      },
      runtime: {
        loading: {
          title: 'Loading server configuration',
          description: 'Reading the current API runtime settings.',
        },
        loadFailed: {
          title: 'Server configuration unavailable',
          description: 'Check that the API server is reachable.',
        },
        sources: {
          title: 'Song sources',
          description: 'Configured by MUSICDL_SOURCES.',
        },
        urlSources: {
          title: 'URL import sources',
          description: 'Configured by MUSICDL_URL_SOURCES.',
        },
        metadata: {
          title: 'Metadata storage',
          description: 'Configured by DB_PATH.',
        },
        storage: {
          title: 'Audio and cover storage',
          description: 'Configured by STORAGE_BACKEND.',
          backends: {
            fs: 'Filesystem',
            s3: 'S3 compatible',
          },
          fsRootLabel: 'Storage directory',
          bucketLabel: 'Bucket',
          endpointLabel: 'Server URL',
          regionLabel: 'Region',
          accessKeyLabel: 'Access Key ID',
          secretKeyLabel: 'Secret Access Key',
        },
        imports: {
          title: 'Import worker',
          description: 'Configured by MUSICDL_* and IMPORT_* values.',
        },
        ai: {
          title: 'AI providers',
          description: 'Configure provider API keys, default provider, and default model.',
          defaultProvider: 'Default provider',
          defaultTag: 'Default',
        },
        keys: {
          backend: 'Backend',
        },
        values: {
          defaultEndpoint: 'AWS default endpoint',
        },
        credentials: {
          configured: 'S3 credentials are configured.',
          missing: 'S3 credentials are missing.',
          keepExisting: 'Leave blank to keep existing',
        },
        apiKeys: {
          configured: 'API key is configured.',
          configuredByEnvironment: 'API key is provided by {env}.',
          configuredByProvider: 'API key is provided by {env}.',
          missing: 'API key is missing.',
          keepExisting: 'Leave blank to keep existing',
          remove: 'Remove saved key',
          keep: 'Keep saved key',
          removePending: 'API key will be removed when saved.',
          removePendingLabel: 'Will remove',
          configuredBadge: 'Configured',
          missingBadge: 'No key',
          envBadge: 'From env',
        },
        save: {
          action: 'Save overrides',
          saving: 'Saving...',
          saved: 'Overrides saved.',
          savedRestart: 'Overrides saved. Restart API and worker to apply startup settings.',
          failed: 'Failed to save overrides',
        },
      },
    },
    library: {
      searchPlaceholder: 'Search',
      loadFailed: 'Failed to load tracks.',
      noResultsTitle: 'No results',
      noResultsHint: 'Try a different keyword',
      searchInExplore: 'Search in explore',
      emptyTitle: 'No music yet',
      emptyHint: 'Upload or download tracks to start listening',
      playTrack: 'Play {title}',
      pauseTrack: 'Pause {title}',
      selectedCount: '{n} selected',
      inPlaylists: 'In: {names}',
      sort: {
        label: 'Sort',
        manual: 'Custom order',
        addedDesc: 'Recently added',
        nameAsc: 'Name (A–Z)',
        nameDesc: 'Name (Z–A)',
        artistAsc: 'Artist (A–Z)',
        durationDesc: 'Longest first',
        durationAsc: 'Shortest first',
      },
    },
    playlist: {
      playAll: 'Play',
      shuffle: 'Shuffle',
      addTracks: 'Add tracks',
      deletePlaylist: 'Delete playlist',
      editDetails: 'Edit details',
      trackCount: '{n} tracks',
      empty: 'Add tracks from the library to start playing this playlist.',
      searchPlaceholder: 'Search playlists',
      sortBy: 'Sort by',
      sortNameAsc: 'Name (A–Z)',
      sortNameDesc: 'Name (Z–A)',
      sortUpdatedDesc: 'Recently updated',
      sortCreatedDesc: 'Recently created',
      sortTracksDesc: 'Most tracks',
      newPlaylist: 'New playlist',
      trackSort: {
        manual: 'Playlist order',
        addedDesc: 'Recently added',
        nameAsc: 'Name (A–Z)',
        nameDesc: 'Name (Z–A)',
        artistAsc: 'Artist (A–Z)',
        durationDesc: 'Longest first',
        durationAsc: 'Shortest first',
      },
    },
    upload: {
      failed: 'Failed to upload. Please retry.',
      chooseFile: 'Please choose a file to upload.',
      dropHint: 'Drop audio files here',
      browseHint: 'or tap to browse',
      success: 'Upload successful!',
    },
    parse: {
      urlPlaceholder: 'Paste a Bilibili or YouTube URL',
      urlLabel: 'Bilibili or YouTube URL',
      unknownUploader: 'Unknown uploader',
      parseFailed: 'Parse failed',
      importFailed: 'Import failed',
      emptyTitle: 'Parse from URL',
      emptyText: 'Paste a Bilibili or YouTube link to extract its audio.',
      status: {
        queued: 'Queued: {song}',
        downloadingPercent: 'Downloading... {percent}%',
        downloadingBytes: 'Downloading... {bytes}',
        downloading: 'Downloading...',
        doneWithSpec: 'Done · {spec}',
        done: 'Done',
        failed: 'Failed',
      },
    },
    import: {
      allSources: 'All',
      searchPlaceholder: 'Search songs, artists...',
      searchLabel: 'Search songs or artists',
      sourceFilter: 'Source filter',
      searchFailed: 'Search failed. Please retry.',
      importFailed: 'Import failed.',
      enterKeyword: 'Enter a keyword to search.',
      emptyText: 'Search across multiple music sources',
      noResults: 'No results found',
      noEnabledResults: 'All results are from disabled sources. Enable the source in Settings.',
      titleAlreadyInLibrary: 'Already in your library',
      titlePossibleMatch: 'Possibly already in your library',
      badges: {
        importing: 'Importing',
        inLibrary: 'In library',
        possibleMatch: 'Possible match',
        download: 'Download',
        unavailable: 'Unavailable',
      },
      loadMore: 'Load more ({count} per source)',
      loadingMore: 'Loading more...',
      status: {
        queued: 'Queued: {song}',
        importingPercent: 'Importing: {song}... {percent}%',
        importingBytes: 'Importing: {song}... {bytes}',
        importing: 'Importing: {song}...',
        doneWithSpec: 'Done: {song} · {spec}',
        done: 'Done: {song}',
        failed: 'Failed: {song}',
        multiple: 'Importing {count} tracks...',
      },
    },
    player: {
      noTrackSelected: 'No track selected',
      unknownArtist: 'Unknown Artist',
      noSyncedLyric: 'No synced lyric',
      noLyricsAvailable: 'No lyrics available',
      trackCover: 'Track cover',
      playModeLabel: 'Play mode: {mode}',
      playModes: {
        sequence: 'sequence',
        repeatAll: 'repeat all',
        repeatOne: 'repeat one',
        shuffle: 'shuffle',
      },
      progressValue: '{current} of {total}',
      openPlayer: 'Open player',
      openPlayerTrack: 'Open player · {title}',
      playFromTime: 'Play from {time}',
      toggleQueue: 'Toggle queue',
      lyrics: {
        offsetLabel: 'Timestamp shift amount',
        shiftAmount: 'Shift',
        earlier: 'Move LRC timestamps earlier',
        later: 'Move LRC timestamps later',
        showTools: 'Adjust lyric timestamps',
        hideTools: 'Hide lyric timestamp tools',
        saveFailed: 'Failed to save lyrics',
      },
      queue: {
        title: 'Queue',
        nowPlaying: 'Now playing',
        upNext: 'Next in queue',
        nextFromLibrary: 'Next from library',
        nextFromPlaylist: 'Next from playlist',
        clear: 'Clear',
        empty: 'Queue is empty. Play a track or add one to the queue.',
        nothingPlaying: 'Nothing playing.',
      },
    },
    metadata: {
      title: 'Edit metadata',
      coverPreview: 'Cover preview',
      coverWillBeRemoved: 'Cover will be removed when you save.',
      fields: {
        title: 'Title',
        artists: 'Artists',
        album: 'Album',
        source: 'Source',
        lyrics: 'Lyrics',
      },
      placeholders: {
        title: 'Track title',
        artists: 'Artist / artists',
        album: 'Album',
        source: 'e.g. NeteaseMusicClient',
        lyrics: 'Paste LRC lyrics (with [mm:ss] timestamps) or plain text. Plain text will display statically without scrolling.',
      },
      sourceNone: 'None',
      lyricsModes: {
        synced: 'Synced (LRC)',
        plain: 'Plain text',
        empty: 'Empty',
      },
      lyricsHint: 'LRC timestamps auto-enable synced highlighting. Plain text renders as-is.',
      saveFailed: 'Save failed',
    },
    shader: {
      expand: 'Expand filament tuner',
      collapse: 'Collapse filament tuner',
      title: 'Filament tuner',
      shaderStyle: 'Shader style',
      styles: {
        aurora: 'Aurora',
        lightning: 'Lightning',
        dna: 'DNA',
        circuit: 'Circuit',
      },
      controls: {
        gain: 'Gain',
        count: 'Count',
        amp: 'Amp',
        freq: 'Freq',
        speed: 'Speed',
        pulseDepth: 'Pulse depth',
        pulseFreq: 'Pulse freq (Hz)',
        density: 'Density',
        smoothness: 'Smoothness',
        converge: 'Converge',
      },
    },
  },
  'ja': {
    auth: {
      loginPrompt: 'アクセストークンを入力してください',
      tokenPlaceholder: 'アクセストークン',
      tokenLabel: 'アクセストークン',
      goBack: '戻る',
    },
    nav: {
      library: 'ライブラリ',
      playlists: 'プレイリスト',
      explore: '探す',
      parse: '解析',
      upload: 'アップロード',
      player: 'プレイヤー',
      settings: '設定',
      login: 'ログイン',
    },
    common: {
      unknown: '不明',
      or: 'または',
      actions: {
        search: '検索',
        searching: '検索中',
        parseUrl: 'URLを解析',
        parsing: '解析中',
        reset: 'リセット',
        download: 'ダウンロード',
        downloading: 'ダウンロード中...',
        upload: 'アップロード',
        uploading: 'アップロード中...',
        login: 'ログイン',
        save: '保存',
        saving: '保存中...',
        cancel: 'キャンセル',
        clear: 'クリア',
        close: '閉じる',
        editMetadata: 'メタデータを編集',
        deleteTrack: 'トラックを削除',
        deletingTrack: '削除中',
        playNext: '次に再生',
        addToQueue: 'キューに追加',
        removeFromQueue: 'キューから削除',
        addToPlaylist: 'プレイリストに追加',
        newPlaylist: '新しいプレイリスト...',
        moreOptions: 'その他の操作',
        removeFromPlaylist: 'プレイリストから削除',
        chooseImage: '画像を選択',
        remove: '削除',
        keepCurrent: '現在のカバーを保持',
        play: '再生',
        pause: '一時停止',
        previousTrack: '前のトラック',
        nextTrack: '次のトラック',
        mute: 'ミュート',
        unmute: 'ミュート解除',
        seek: 'シーク',
        volume: '音量',
        copy: 'コピー',
        copied: 'コピーしました',
        failed: '失敗',
      },
    },
    errors: {
      parseFailedStatus: '解析に失敗しました（{status}）',
      updateFailedStatus: '更新に失敗しました（{status}）',
      coverUploadFailedStatus: 'カバーアップロードに失敗しました（{status}）',
      coverDeleteFailedStatus: 'カバー削除に失敗しました（{status}）',
      missingImportJobId: 'インポートジョブIDがありません',
    },
    settings: {
      title: '設定',
      description: 'このブラウザの画面設定を管理します。',
      groups: {
        runtime: '実行構成',
        appearance: '外観',
        playback: '再生エフェクト',
      },
      sections: {
        label: '設定セクション',
      },
      on: 'オン',
      off: 'オフ',
      language: {
        title: '表示言語',
        description: '変更はすぐに反映され、この端末に保存されます。',
        followSystem: 'システムに従う',
        currentValue: '現在 {language} を使用中',
        effectiveLocale: '有効な言語',
      },
      coverEffectEnabled: {
        title: 'カバーホログラム効果',
        description: 'オフにすると、プレイヤーカバーのホログラム効果が非表示になります。',
      },
      coverEffect: {
        title: 'カバーエフェクト',
        description: 'プレイヤーカバーに使用するホログラムスタイルを選択します。',
        currentValue: '現在のエフェクト：{effect}',
        options: {
          amazingRare: 'アメイジングレア',
          radiantHolo: 'ラディアントホロ',
          vMax: 'VMAX',
          vStar: 'VSTAR',
        },
      },
      progressEffectEnabled: {
        title: 'プログレスバーシェーダー',
        description: 'オフにすると、アニメーションシェーダーの代わりにシンプルなプログレスバーが表示されます。',
      },
      runtime: {
        loading: {
          title: 'サーバー設定を読み込み中',
          description: '現在のAPIランタイム設定を読み込んでいます。',
        },
        loadFailed: {
          title: 'サーバー設定を利用できません',
          description: 'APIサーバーに接続できることを確認してください。',
        },
        sources: {
          title: '楽曲ソース',
          description: 'MUSICDL_SOURCES で構成。',
        },
        urlSources: {
          title: 'URLインポートソース',
          description: 'MUSICDL_URL_SOURCES で構成。',
        },
        metadata: {
          title: 'メタデータストレージ',
          description: 'DB_PATH で構成。',
        },
        storage: {
          title: '音声・カバーストレージ',
          description: 'STORAGE_BACKEND で構成。',
          backends: {
            fs: 'ファイルシステム',
            s3: 'S3 互換',
          },
          fsRootLabel: 'ストレージディレクトリ',
          bucketLabel: 'バケット',
          endpointLabel: 'サーバーURL',
          regionLabel: 'リージョン',
          accessKeyLabel: 'アクセスキーID',
          secretKeyLabel: 'シークレットアクセスキー',
        },
        imports: {
          title: 'インポートワーカー',
          description: 'MUSICDL_* および IMPORT_* で構成。',
        },
        ai: {
          title: 'AI プロバイダー',
          description: 'プロバイダーAPIキー、デフォルトプロバイダー、デフォルトモデルを設定します。',
          defaultProvider: 'デフォルトプロバイダー',
          defaultTag: 'デフォルト',
        },
        keys: {
          backend: 'バックエンド',
        },
        values: {
          defaultEndpoint: 'AWS デフォルトエンドポイント',
        },
        credentials: {
          configured: 'S3 認証情報が設定されています。',
          missing: 'S3 認証情報が不足しています。',
          keepExisting: '空白のままにすると既存の値が保持されます',
        },
        apiKeys: {
          configured: 'APIキーが設定されています。',
          configuredByEnvironment: 'APIキーは {env} から提供されています。',
          configuredByProvider: 'APIキーは {env} から提供されています。',
          missing: 'APIキーがありません。',
          keepExisting: '空白のままにすると既存の値が保持されます',
          remove: '保存済みキーを削除',
          keep: '保存済みキーを保持',
          removePending: '保存時にAPIキーが削除されます。',
          removePendingLabel: '削除予定',
          configuredBadge: '設定済み',
          missingBadge: 'キーなし',
          envBadge: '環境変数',
        },
        save: {
          action: '上書きを保存',
          saving: '保存中...',
          saved: '上書きが保存されました。',
          savedRestart: '上書きが保存されました。起動時設定を反映するにはAPIとワーカーを再起動してください。',
          failed: '上書きの保存に失敗しました',
        },
      },
    },
    library: {
      searchPlaceholder: '検索',
      loadFailed: 'トラックの読み込みに失敗しました。',
      noResultsTitle: '結果がありません',
      noResultsHint: '別のキーワードをお試しください',
      searchInExplore: '探索で検索',
      emptyTitle: 'まだ音楽がありません',
      emptyHint: 'トラックをアップロードまたはダウンロードしてください',
      playTrack: '{title} を再生',
      pauseTrack: '{title} を一時停止',
      selectedCount: '{n} 件選択中',
      inPlaylists: '所属：{names}',
      sort: {
        label: '並び替え',
        manual: 'カスタム順',
        addedDesc: '最近追加',
        nameAsc: '名前（A-Z）',
        nameDesc: '名前（Z-A）',
        artistAsc: 'アーティスト（A-Z）',
        durationDesc: '長い順',
        durationAsc: '短い順',
      },
    },
    playlist: {
      playAll: '再生',
      shuffle: 'シャッフル',
      addTracks: 'トラックを追加',
      deletePlaylist: 'プレイリストを削除',
      editDetails: '詳細を編集',
      trackCount: '{n} 曲',
      empty: 'ライブラリからトラックを追加してください。',
      searchPlaceholder: 'プレイリストを検索',
      sortBy: '並び替え',
      sortNameAsc: '名前（A-Z）',
      sortNameDesc: '名前（Z-A）',
      sortUpdatedDesc: '最近更新',
      sortCreatedDesc: '最近作成',
      sortTracksDesc: '曲数が多い順',
      newPlaylist: '新しいプレイリスト',
      trackSort: {
        manual: 'プレイリスト順',
        addedDesc: '最近追加',
        nameAsc: '名前（A-Z）',
        nameDesc: '名前（Z-A）',
        artistAsc: 'アーティスト（A-Z）',
        durationDesc: '長い順',
        durationAsc: '短い順',
      },
    },
    upload: {
      failed: 'アップロードに失敗しました。再試行してください。',
      chooseFile: 'アップロードするファイルを選択してください。',
      dropHint: 'ここに音声ファイルをドロップ',
      browseHint: 'またはタップして選択',
      success: 'アップロード成功！',
    },
    parse: {
      urlPlaceholder: 'Bilibili または YouTube のURLを貼り付け',
      urlLabel: 'Bilibili または YouTube のURL',
      unknownUploader: '不明なアップローダー',
      parseFailed: '解析に失敗しました',
      importFailed: 'インポートに失敗しました',
      emptyTitle: 'URLから解析',
      emptyText: 'Bilibili または YouTube のリンクを貼り付けて音声を抽出します。',
      status: {
        queued: 'キューイング済み：{song}',
        downloadingPercent: 'ダウンロード中... {percent}%',
        downloadingBytes: 'ダウンロード中... {bytes}',
        downloading: 'ダウンロード中...',
        doneWithSpec: '完了 · {spec}',
        done: '完了',
        failed: '失敗',
      },
    },
    import: {
      allSources: 'すべて',
      searchPlaceholder: '曲、アーティストを検索...',
      searchLabel: '曲またはアーティストを検索',
      sourceFilter: 'ソースフィルター',
      searchFailed: '検索に失敗しました。再試行してください。',
      importFailed: 'インポートに失敗しました。',
      enterKeyword: '検索キーワードを入力してください。',
      emptyText: '複数の音楽ソースを横断検索',
      noResults: '検索結果がありません',
      noEnabledResults: 'すべての検索結果は無効化されたソースからのものです。設定でソースを有効にしてください。',
      titleAlreadyInLibrary: 'すでにライブラリにあります',
      titlePossibleMatch: 'ライブラリに存在する可能性があります',
      badges: {
        importing: 'インポート中',
        inLibrary: 'ライブラリ内',
        possibleMatch: '重複の可能性',
        download: 'ダウンロード',
        unavailable: '利用不可',
      },
      loadMore: 'さらに読み込む（ソースあたり {count} 件）',
      loadingMore: 'さらに読み込み中...',
      status: {
        queued: 'キューイング済み：{song}',
        importingPercent: 'インポート中：{song}... {percent}%',
        importingBytes: 'インポート中：{song}... {bytes}',
        importing: 'インポート中：{song}...',
        doneWithSpec: '完了：{song} · {spec}',
        done: '完了：{song}',
        failed: '失敗：{song}',
        multiple: '{count} 件のトラックをインポート中...',
      },
    },
    player: {
      noTrackSelected: 'トラックが選択されていません',
      unknownArtist: '不明なアーティスト',
      noSyncedLyric: '同期歌詞なし',
      noLyricsAvailable: '歌詞がありません',
      trackCover: 'トラックカバー',
      playModeLabel: '再生モード：{mode}',
      playModes: {
        sequence: '順次再生',
        repeatAll: '全曲リピート',
        repeatOne: '1曲リピート',
        shuffle: 'シャッフル',
      },
      progressValue: '{current} / {total}',
      openPlayer: 'プレイヤーを開く',
      openPlayerTrack: 'プレイヤーを開く · {title}',
      playFromTime: '{time} から再生',
      toggleQueue: 'キューを切り替え',
      lyrics: {
        offsetLabel: 'タイムスタンプオフセット量',
        shiftAmount: 'シフト',
        earlier: 'LRCタイムスタンプを早める',
        later: 'LRCタイムスタンプを遅らせる',
        showTools: '歌詞タイムスタンプを調整',
        hideTools: '歌詞タイムスタンプ調整を隠す',
        saveFailed: '歌詞の保存に失敗しました',
      },
      queue: {
        title: '再生キュー',
        nowPlaying: '再生中',
        upNext: '次に再生',
        nextFromLibrary: 'ライブラリから',
        nextFromPlaylist: 'プレイリストから',
        clear: 'クリア',
        empty: 'キューは空です。トラックを再生するかキューに追加してください。',
        nothingPlaying: '再生中のトラックはありません。',
      },
    },
    metadata: {
      title: 'メタデータを編集',
      coverPreview: 'カバープレビュー',
      coverWillBeRemoved: '保存時にカバーが削除されます。',
      fields: {
        title: 'タイトル',
        artists: 'アーティスト',
        album: 'アルバム',
        source: 'ソース',
        lyrics: '歌詞',
      },
      placeholders: {
        title: 'トラックタイトル',
        artists: 'アーティスト',
        album: 'アルバム',
        source: '例：NeteaseMusicClient',
        lyrics: 'LRC歌詞（[mm:ss] タイムスタンプ付き）またはプレーンテキストを貼り付け。プレーンテキストはスクロールせずに表示されます。',
      },
      sourceNone: 'なし',
      lyricsModes: {
        synced: '同期歌詞（LRC）',
        plain: 'プレーンテキスト',
        empty: '空',
      },
      lyricsHint: 'タイムスタンプ付きLRCは自動で同期ハイライトが有効になります。プレーンテキストはそのまま表示されます。',
      saveFailed: '保存に失敗しました',
    },
    shader: {
      expand: 'フィラメントチューナーを展開',
      collapse: 'フィラメントチューナーを折りたたむ',
      title: 'フィラメントチューナー',
      shaderStyle: 'シェーダースタイル',
      styles: {
        aurora: 'オーロラ',
        lightning: '稲妻',
        dna: 'DNA',
        circuit: '回路',
      },
      controls: {
        gain: 'ゲイン',
        count: '数',
        amp: '振幅',
        freq: '周波数',
        speed: '速度',
        pulseDepth: 'パルス深度',
        pulseFreq: 'パルス周波数 (Hz)',
        density: '密度',
        smoothness: '滑らかさ',
        converge: '収束',
      },
    },
  },
  'zh-CN': {
    auth: {
      loginPrompt: '请输入访问令牌以继续',
      tokenPlaceholder: '访问令牌',
      tokenLabel: '访问令牌',
      goBack: '返回',
    },
    nav: {
      library: '音乐库',
      playlists: '歌单',
      explore: '探索',
      parse: '解析',
      upload: '上传',
      player: '播放器',
      settings: '设置',
      login: '登录',
    },
    common: {
      unknown: '未知',
      or: '或',
      actions: {
        search: '搜索',
        searching: '搜索中',
        parseUrl: '解析链接',
        parsing: '解析中',
        reset: '重置',
        download: '下载',
        downloading: '下载中...',
        upload: '上传',
        uploading: '上传中...',
        login: '登录',
        save: '保存',
        saving: '保存中...',
        cancel: '取消',
        clear: '清除',
        close: '关闭',
        editMetadata: '编辑元数据',
        deleteTrack: '删除音轨',
        deletingTrack: '删除中',
        playNext: '下一首播放',
        addToQueue: '添加到队列',
        removeFromQueue: '从队列移除',
        addToPlaylist: '添加到歌单',
        newPlaylist: '新建歌单...',
        moreOptions: '更多操作',
        removeFromPlaylist: '从歌单移除',
        chooseImage: '选择图片',
        remove: '移除',
        keepCurrent: '保留当前封面',
        play: '播放',
        pause: '暂停',
        previousTrack: '上一首',
        nextTrack: '下一首',
        mute: '静音',
        unmute: '取消静音',
        seek: '调整进度',
        volume: '音量',
        copy: '复制',
        copied: '已复制',
        failed: '失败',
      },
    },
    errors: {
      parseFailedStatus: '解析失败（{status}）',
      updateFailedStatus: '更新失败（{status}）',
      coverUploadFailedStatus: '封面上传失败（{status}）',
      coverDeleteFailedStatus: '封面删除失败（{status}）',
      missingImportJobId: '缺少导入任务 ID',
    },
    settings: {
      title: '设置',
      description: '管理当前浏览器中的界面偏好。',
      groups: {
        runtime: '运行配置',
        appearance: '外观',
        playback: '播放器特效',
      },
      sections: {
        label: '设置分区',
      },
      on: '开启',
      off: '关闭',
      language: {
        title: '界面语言',
        description: '修改会立即生效，并保存在当前设备本地。',
        followSystem: '跟随系统',
        currentValue: '当前使用 {language}',
        effectiveLocale: '当前生效语言',
      },
      coverEffectEnabled: {
        title: '封面闪卡特效',
        description: '关闭后，播放器封面不再叠加全息闪卡效果。',
      },
      coverEffect: {
        title: '封面样式',
        description: '选择播放器封面使用的闪卡样式。',
        currentValue: '当前特效：{effect}',
        options: {
          amazingRare: 'Amazing Rare',
          radiantHolo: 'Radiant 闪',
          vMax: 'VMAX',
          vStar: 'VSTAR',
        },
      },
      progressEffectEnabled: {
        title: '进度条着色器特效',
        description: '关闭后使用简洁进度条替代动态着色器。',
      },
      runtime: {
        loading: {
          title: '正在加载服务端配置',
          description: '读取当前 API 运行设置。',
        },
        loadFailed: {
          title: '无法读取服务端配置',
          description: '请确认 API 服务可以访问。',
        },
        sources: {
          title: '歌曲来源',
          description: '由 MUSICDL_SOURCES 配置。',
        },
        urlSources: {
          title: '链接导入来源',
          description: '由 MUSICDL_URL_SOURCES 配置。',
        },
        metadata: {
          title: '元数据存储',
          description: '由 DB_PATH 配置。',
        },
        storage: {
          title: '音频与封面存储',
          description: '由 STORAGE_BACKEND 配置。',
          backends: {
            fs: '文件系统',
            s3: 'S3 兼容',
          },
          fsRootLabel: '存储目录',
          bucketLabel: '存储桶',
          endpointLabel: '服务器地址',
          regionLabel: '区域',
          accessKeyLabel: '访问密钥 ID',
          secretKeyLabel: '秘密访问密钥',
        },
        imports: {
          title: '导入 worker',
          description: '由 MUSICDL_* 和 IMPORT_* 值配置。',
        },
        ai: {
          title: 'AI 提供商',
          description: '配置提供商 API Key、默认提供商和默认模型。',
          defaultProvider: '默认提供商',
          defaultTag: '默认',
        },
        keys: {
          backend: '后端',
        },
        values: {
          defaultEndpoint: 'AWS 默认端点',
        },
        credentials: {
          configured: 'S3 凭据已配置。',
          missing: '缺少 S3 凭据。',
          keepExisting: '留空则保留现有值',
        },
        apiKeys: {
          configured: 'API Key 已配置。',
          configuredByEnvironment: 'API Key 由 {env} 提供。',
          configuredByProvider: 'API Key 由 {env} 提供。',
          missing: '缺少 API Key。',
          keepExisting: '留空则保留现有值',
          remove: '移除已保存 Key',
          keep: '保留已保存 Key',
          removePending: '保存后将移除 API Key。',
          removePendingLabel: '将移除',
          configuredBadge: '已配置',
          missingBadge: '无 Key',
          envBadge: '环境变量',
        },
        save: {
          action: '保存覆盖',
          saving: '保存中...',
          saved: '覆盖已保存。',
          savedRestart: '覆盖已保存。重启 API 和 worker 后启动期配置会完整生效。',
          failed: '保存覆盖失败',
        },
      },
    },
    library: {
      searchPlaceholder: '搜索',
      loadFailed: '加载音轨失败。',
      noResultsTitle: '没有结果',
      noResultsHint: '试试别的关键词',
      searchInExplore: '探索曲库',
      emptyTitle: '还没有音乐',
      emptyHint: '上传或下载音轨后即可开始播放',
      playTrack: '播放 {title}',
      pauseTrack: '暂停 {title}',
      selectedCount: '已选择 {n} 项',
      inPlaylists: '所属：{names}',
      sort: {
        label: '排序',
        manual: '自定义顺序',
        addedDesc: '最近添加',
        nameAsc: '名称（A-Z）',
        nameDesc: '名称（Z-A）',
        artistAsc: '艺术家（A-Z）',
        durationDesc: '时长降序',
        durationAsc: '时长升序',
      },
    },
    playlist: {
      playAll: '播放',
      shuffle: '随机播放',
      addTracks: '添加曲目',
      deletePlaylist: '删除歌单',
      editDetails: '编辑信息',
      trackCount: '共 {n} 首',
      empty: '从音乐库添加曲目即可开始播放这个歌单。',
      searchPlaceholder: '搜索歌单',
      sortBy: '排序',
      sortNameAsc: '名称（A-Z）',
      sortNameDesc: '名称（Z-A）',
      sortUpdatedDesc: '最近更新',
      sortCreatedDesc: '最近创建',
      sortTracksDesc: '曲目数最多',
      newPlaylist: '新建歌单',
      trackSort: {
        manual: '歌单顺序',
        addedDesc: '最近添加',
        nameAsc: '名称（A-Z）',
        nameDesc: '名称（Z-A）',
        artistAsc: '艺术家（A-Z）',
        durationDesc: '时长降序',
        durationAsc: '时长升序',
      },
    },
    upload: {
      failed: '上传失败，请重试。',
      chooseFile: '请选择一个要上传的文件。',
      dropHint: '将音频文件拖到这里',
      browseHint: '或点击选择文件',
      success: '上传成功！',
    },
    parse: {
      urlPlaceholder: '粘贴 Bilibili 或 YouTube 链接',
      urlLabel: 'Bilibili 或 YouTube 链接',
      unknownUploader: '未知上传者',
      parseFailed: '解析失败',
      importFailed: '导入失败',
      emptyTitle: '从链接解析',
      emptyText: '粘贴 Bilibili 或 YouTube 链接以提取音频。',
      status: {
        queued: '已排队：{song}',
        downloadingPercent: '下载中... {percent}%',
        downloadingBytes: '下载中... {bytes}',
        downloading: '下载中...',
        doneWithSpec: '完成 · {spec}',
        done: '完成',
        failed: '失败',
      },
    },
    import: {
      allSources: '全部',
      searchPlaceholder: '搜索歌曲、歌手...',
      searchLabel: '搜索歌曲或歌手',
      sourceFilter: '来源筛选',
      searchFailed: '搜索失败，请重试。',
      importFailed: '导入失败。',
      enterKeyword: '请输入搜索关键词。',
      emptyText: '跨多个音乐源搜索',
      noResults: '没有搜索结果',
      noEnabledResults: '所有结果来自已禁用的源，请在设置中启用对应源。',
      titleAlreadyInLibrary: '已在音乐库中',
      titlePossibleMatch: '可能已在音乐库中',
      badges: {
        importing: '导入中',
        inLibrary: '已在库中',
        possibleMatch: '可能重复',
        download: '下载',
        unavailable: '不可用',
      },
      loadMore: '加载更多（每个来源 {count} 条）',
      loadingMore: '加载更多中...',
      status: {
        queued: '已排队：{song}',
        importingPercent: '导入中：{song}... {percent}%',
        importingBytes: '导入中：{song}... {bytes}',
        importing: '导入中：{song}...',
        doneWithSpec: '完成：{song} · {spec}',
        done: '完成：{song}',
        failed: '失败：{song}',
        multiple: '正在导入 {count} 首音轨...',
      },
    },
    player: {
      noTrackSelected: '未选择音轨',
      unknownArtist: '未知艺术家',
      noSyncedLyric: '没有同步歌词',
      noLyricsAvailable: '暂无歌词',
      trackCover: '音轨封面',
      playModeLabel: '播放模式：{mode}',
      playModes: {
        sequence: '顺序播放',
        repeatAll: '列表循环',
        repeatOne: '单曲循环',
        shuffle: '随机播放',
      },
      progressValue: '{current} / {total}',
      openPlayer: '打开播放器',
      openPlayerTrack: '打开播放器 · {title}',
      playFromTime: '从 {time} 播放',
      toggleQueue: '切换队列',
      lyrics: {
        offsetLabel: '时间戳偏移量',
        shiftAmount: '移动',
        earlier: '提前 LRC 时间戳',
        later: '延后 LRC 时间戳',
        showTools: '调整歌词时间戳',
        hideTools: '隐藏歌词时间戳工具',
        saveFailed: '保存歌词失败',
      },
      queue: {
        title: '播放队列',
        nowPlaying: '正在播放',
        upNext: '下一首',
        nextFromLibrary: '来自音乐库',
        nextFromPlaylist: '来自歌单',
        clear: '清空',
        empty: '队列是空的。点击播放或添加歌曲到队列。',
        nothingPlaying: '当前没有正在播放的音轨。',
      },
    },
    metadata: {
      title: '编辑元数据',
      coverPreview: '封面预览',
      coverWillBeRemoved: '保存后会移除封面。',
      fields: {
        title: '标题',
        artists: '艺术家',
        album: '专辑',
        source: '来源',
        lyrics: '歌词',
      },
      placeholders: {
        title: '音轨标题',
        artists: '艺术家',
        album: '专辑',
        source: '例如 NeteaseMusicClient',
        lyrics: '粘贴 LRC 歌词（包含 [mm:ss] 时间戳）或纯文本歌词。纯文本会静态显示，不会跟随滚动。',
      },
      sourceNone: '无',
      lyricsModes: {
        synced: '同步歌词（LRC）',
        plain: '纯文本',
        empty: '空',
      },
      lyricsHint: '带时间戳的 LRC 会自动启用同步高亮。纯文本会按原样显示。',
      saveFailed: '保存失败',
    },
    shader: {
      expand: '展开丝带调节器',
      collapse: '收起丝带调节器',
      title: '丝带调节器',
      shaderStyle: '着色器样式',
      styles: {
        aurora: '极光',
        lightning: '闪电',
        dna: 'DNA',
        circuit: '电路',
      },
      controls: {
        gain: '增益',
        count: '数量',
        amp: '振幅',
        freq: '频率',
        speed: '速度',
        pulseDepth: '脉冲深度',
        pulseFreq: '脉冲频率 (Hz)',
        density: '密度',
        smoothness: '平滑度',
        converge: '收敛度',
      },
    },
  },
} as const
