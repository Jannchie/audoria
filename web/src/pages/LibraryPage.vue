  ...
  <section class="space-y-8">
    <header class="flex gap-4 items-center justify-between">
      ...
      <div class="text-right">
        ...
        <RouterLink
          class="text-sm text-white mt-2 px-3 py-2 border border-[var(--accent)]/70 rounded-lg bg-[var(--accent)]/10 inline-flex gap-2 items-center hover:bg-[var(--accent)]/20"
          to="/upload"
        >
    ...
    <div class="px-4 py-3 border border-[var(--line)] rounded-xl bg-[var(--bg-surface)]/70 flex flex-wrap gap-3 items-center">
      <div class="flex flex-wrap gap-2">
        <button
          ...
          :class="tab === 'Songs'
            ...
            : 'border-[var(--line)] text-slate-400 hover:bg-[var(--bg-muted)]/70'"
          type="button"
        >
          {{ tab }}
        </button>
      ...
      <div class="flex-1 min-w-[240px] relative">
        ...
        <input
          v-model="search"
          class="text-sm text-slate-100 py-2 pl-10 pr-4 border border-[var(--line)] rounded-lg bg-[var(--bg-primary)]/60 w-full placeholder:text-slate-600 focus:outline-none focus:border-[var(--accent)]"
          placeholder="Search by filename"
          type="search"
        >
      </div>
      <div class="text-xs text-slate-400 flex gap-2 items-center">
        <span class="px-3 py-2 border border-[var(--line)] rounded-full bg-[var(--bg-primary)]/60">Filters</span>
        <span class="px-3 py-2 border border-[var(--line)] rounded-full bg-[var(--bg-primary)]/60">Newest</span>
      </div>
    </div>

    <div class="border border-[var(--line)] rounded-xl bg-[var(--bg-surface)]/70 overflow-hidden divide-[var(--line)] divide-y">
      ...
      <button
        ...
        v-else
        :key="track.id"
        class="group px-5 py-4 text-left flex gap-4 w-full transition-colors items-center hover:bg-[var(--bg-muted)]/60"
        :class="currentTrackId === track.id ? 'border-l-2 border-[var(--accent)]/80' : 'border-l-2 border-transparent'"
        @click="selectTrack(track.id)"
      >
        <div class="text-base text-white font-semibold rounded-lg bg-[var(--bg-muted)] flex h-14 w-14 items-center justify-center">
        ...
        <div class="flex-1 min-w-0">
          <div class="flex gap-2 items-center">
            ...
            <span class="text-[11px] text-slate-400 px-2 py-1 border border-[var(--line)] rounded-full">
              {{ track.contentType || 'audio' }}
            </span>
          </div>
          <div class="text-xs text-slate-400 mt-2 flex flex-wrap gap-2 items-center">
            <span class="px-2 py-1 border border-[var(--line)] rounded-full bg-[var(--bg-primary)]/60">
              {{ vibeFor(track) }}
            </span>
            <span class="px-2 py-1 border border-[var(--line)] rounded-full bg-[var(--bg-primary)]/60">
        ...
        <div
          ...
          :class="currentTrackId === track.id
            ? 'border-[var(--accent)]/70 bg-[var(--accent)]/15 text-white'
            : 'border-[var(--line)] bg-[var(--bg-primary)]/60 text-slate-300'"
        >
